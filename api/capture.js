...
export default async function handler(req,res){
  ...
  const target=Number(process.env.TARGET_DPH||35),decision=decisionForOffer({dollarsPerHour:rate?.dollarsPerHour,target,mode}),verdict=decision.verdict;
  return res.status(200).json({ok:true,persisted:Boolean(saved),id:saved?.[0]?.id||null,parsed,rate,decision,verdict,zone,vehicle,timeBlock,marketCell,mode});
}