...
export default async function handler(req,res){
 ...
 try{
  ...
  const currentBlock=blockName(new Date()),blockCompleted=todayCompleted.filter(o=>blockName(o.captured_at)===currentBlock),blockEarnings=blockCompleted.reduce((sum,o)=>sum+Number(o.final_payout??o.payout??0),0);
  ...
  return res.status(200).json({activeShift:active,onlineMinutes:Number(onlineMinutes.toFixed(1)),idleMinutes:Number(idleMinutes.toFixed(1)),offers:signalOffers.length,completed:completed.length,totalEarnings:Number(totalEarnings.toFixed(2)),todayEarnings:Number(todayEarnings.toFixed(2)),blockEarnings:Number(blockEarnings.toFixed(2)),currentBlock,offerPayout:Number(offerPayout.toFixed(2)),importedEarnings:Number(imported.toFixed(2)),onlineDph:onlineMinutes?Number((totalEarnings/(onlineMinutes/60)).toFixed(2)):0,monthlyGoal:Number(goals?.[0]?.monthly_goal||4000),merchantStats,zones:Object.values(zones),recentEvents:(events||[]).slice(-100)});
