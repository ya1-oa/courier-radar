...
export function initRadarMap(){if(ready)return ready;ready=new Promise((resolve,reject)=>{try{
 ...
 map.on('load',()=>{
  ...
  map.addLayer({id:'pickup-cluster-count',type:'symbol',source:'radar-pickups',filter:['has','point_count'],layout:{'text-field':['get','point_count_abbreviated'],'text-size':11,'text-font':['Noto Sans Regular']},paint:{'text-color':'#001d16'}});
  ...
  map.addSource('radar-destination',{type:'geojson',data:collection([])});
...
export async function updateRadarMap({features=[],position=null,destination=null,history=[],fit=false}={}){
